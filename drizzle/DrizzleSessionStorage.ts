...
export class DrizzleSessionStoragePostgres implements SessionStorage {
    ...
    constructor(
        ...
        private readonly sessionsTable: ShopifySessionTable,
    ) {}
    ...
    public async loadSession(id: string): Promise<Session | undefined> {
        try {
            const row = await this.db.select().from(this.sessionsTable).where(eq(this.sessionsTable.id, id));
    ...
    public async deleteSession(id: string): Promise<boolean> {
        try {
            await this.db.delete(this.sessionsTable).where(eq(this.sessionsTable.id, id));
        } catch (error) {
            console.error(error);
    ...
    public async deleteSessions(ids: string[]): Promise<boolean> {
        try {
            await this.db.delete(this.sessionsTable).where(inArray(this.sessionsTable.id, ids));
    ...
    public async findSessionsByShop(shop: string): Promise<Session[]> {
        const sessions = await this.db.select().from(this.sessionsTable).where(eq(this.sessionsTable.shop, shop)).orderBy(desc(this.sessionsTable.expires));

        return sessions.map((session) => this.rowToSession(session));
    }

    private sessionToRow(session: Session): InferInsertModel<ShopifySessionTable> {
        return {
            id: session.id,
            shop: session.shop,
            state: session.state,
